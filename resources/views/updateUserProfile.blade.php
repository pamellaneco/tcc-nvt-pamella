                    ...
                    <div class="row mt-5 justify-content-center">
                        <div class="col-md-6">
                            <div id="form-card" class="card mt-5 bg-success text-white">
                                ...
                                <div class="card-body">
                                    <form class="col-md-12" id="form-posts" action="/profile/update/{{$profile['id']}}" method="POST" enctype="multipart/form-data">
                                        @csrf
                                        <div class="m-1">
                                            <label class="mt-2 col-md-8 col-form-label text-md-end" for="name">{{ __('Nome:') }}</label>
                                            <input class="mt-2 form-control @error('name') is-invalid @enderror" type="text" name="name" value="{{$profile['name']}}" placeholder="Seu nome...">
                                        </div>
                                        <div class="m-1">
                                            <label class="mt-2 col-md-8 col-form-label text-md-end" for="email">{{ __('Email:') }}</label>
                                            <input class="mt-2 form-control @error('email') is-invalid @enderror" type="email" name="email" value="{{$profile['email']}}" placeholder="Seu Email...">
                                        </div>
                                        <div class="m-1">
                                            <label class="mt-2 col-md-8 col-form-label text-md-end" for="phone">{{ __('Telefone:') }}</label>
                                            <input class="mt-2 form-control @error('phone') is-invalid @enderror" type="number" name="phone" value="{{$profile['phone']}}" placeholder="Seu telefone...">
                                        </div>
                                        <div class="m-1">
                                            <label class="mt-2 col-md-8 col-form-label text-md-end" for="place">{{ __('Localidade:') }}</label>
                                            <input class="mt-2 form-control @error('place') is-invalid @enderror" type="text" name="place" value="{{$profile['place']}}" placeholder="Seu endereço (cidade)...">
                                        </div>
